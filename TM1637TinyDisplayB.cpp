...
TM1637TinyDisplayB::TM1637TinyDisplayB(uint8_t pinClk, uint8_t pinDIO, unsigned int bitDelay, 
...
void TM1637TinyDisplayB::flipDisplay(bool flip)
{
  m_flipDisplay = flip;
  writeBuffer();
}
...
void TM1637TinyDisplayB::writeBuffer()
{
  uint8_t dot = 0;
...
void TM1637TinyDisplayB::setSegments(const uint8_t segments[], uint8_t length, uint8_t pos)
{
...
void TM1637TinyDisplayB::showNumber(int num, bool leading_zero, uint8_t length, uint8_t pos)
{
  showNumber(long(num), leading_zero, length, pos);
}

void TM1637TinyDisplayB::showNumber(long num, bool leading_zero, uint8_t length, uint8_t pos)
{
  if(leading_zero) {
...
void TM1637TinyDisplayB::showNumber(double num, uint8_t decimal_length, uint8_t length, uint8_t pos)
{
  int num_len = 0;              
...
void TM1637TinyDisplayB::showNumberDec(int num, uint8_t dots, bool leading_zero,
                                    uint8_t length, uint8_t pos)
{
  showNumberBaseEx(num < 0? -10 : 10, num < 0? -num : num, dots, leading_zero, length, pos);
}

void TM1637TinyDisplayB::showNumberHex(uint16_t num, uint8_t dots, bool leading_zero,
                                    uint8_t length, uint8_t pos)
{
  showNumberBaseEx(16, num, dots, leading_zero, length, pos);
}

void TM1637TinyDisplayB::showNumberBaseEx(int8_t base, uint16_t num, uint8_t dots, bool leading_zero,
                                    uint8_t length, uint8_t pos)
{
  bool negative = false;
  if (base < 0) {
...
void TM1637TinyDisplayB::showString(const char s[], uint8_t length, uint8_t pos, uint8_t dots)
{
  // digits[MAXDIGITS] output array to render
  memset(digits,0,sizeof(digits));
...
void TM1637TinyDisplayB::showString_P(const char s[], uint8_t length, uint8_t pos, uint8_t dots) 
{
  // digits[MAXDIGITS] output array to render
  memset(digits,0,sizeof(digits));
...
void TM1637TinyDisplayB::showLevel(unsigned int level, bool horizontal) 
{
  // digits[MAXDIGITS] output array to render
  memset(digits,0,sizeof(digits));
...
void TM1637TinyDisplayB::showAnimation(const uint8_t data[][4], unsigned int frames, unsigned int ms)
{
  // Animation sequence
...
void TM1637TinyDisplayB::showAnimation_P(const uint8_t data[][4], unsigned int frames, unsigned int ms)
{
...
void TM1637TinyDisplayB::bitDelay()
{
  delayMicroseconds(m_bitDelay);
}

void TM1637TinyDisplayB::start()
{
  pinMode(m_pinDIO, OUTPUT);
  bitDelay();
}

void TM1637TinyDisplayB::stop()
{
  pinMode(m_pinDIO, OUTPUT);
  bitDelay();
  pinMode(m_pinClk, INPUT);
...
bool TM1637TinyDisplayB::writeByte(uint8_t b)
{
  uint8_t data = b;
...
void TM1637TinyDisplayB::showDots(uint8_t dots, uint8_t* digits)
{
...
uint8_t TM1637TinyDisplayB::encodeDigit(uint8_t digit)
{
  // return digitToSegment[digit & 0x0f] using PROGMEM
  return pgm_read_byte(digitToSegment + (digit & 0x0f));
}

uint8_t TM1637TinyDisplayB::encodeASCII(uint8_t chr)
{
